...
на `401`/`403` для приватных helm-репозиториев. `repo-sync` закрывает этот
...
|-----------|--------------|----------|
| `CMP_REPO_SYNC_TTL` | `300` | Секунды жизни кэша. В течение TTL повторные запуски пропускаются |
| `CMP_REPO_SYNC_OUT` | `/tmp/helm-config/helm/repositories.yaml` | Путь к выходному файлу |
| `CMP_REPO_SYNC_NAMESPACE` | из `/var/run/secrets/.../namespace` | Namespace для чтения Secret'ов |
| `CMP_REPO_SYNC_API_HOST` | `kubernetes.default.svc` | Адрес Kubernetes API |
| `CMP_REPO_SYNC_INSECURE` | `0` | `1`/`true` — отключить проверку TLS (только debug) |

## RBAC

Бинарю нужен ServiceAccount с правом `get`/`list` Secret'ов в namespace
ArgoCD. Готовые манифесты — `../repo-rbac.example.yaml`. Минимально:

```yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
  name: argocd-cmp-repo-sync
rules:
  - apiGroups: [""]
    resources: ["secrets"]
    verbs: ["get", "list"]
...
RoleBinding привязывает этот Role к ServiceAccount, с которым крутится
`argocd-repo-server` (указывается через `repoServer.serviceAccount.name` в
ArgoCD helm-values).

## Сборка

Встроена в `../Dockerfile.sops-cmp` как первая multi-stage стадия (`build-repo-sync`).
Локально для проверки:

```sh
cd tools/repo-sync
go build -o /tmp/repo-sync .
```
